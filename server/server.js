...
const OAUTH_CALLBACK_URL =
  ...
  "http://localhost:3001/oauth/battlenet/callback/";
const APP_LISTEN_PORT = process.env.APP_LISTEN_PORT || 3001;
const MONGO_HOST = process.env.MONGO_HOST || "mongodb://127.0.0.1:27017/posts";
...
app.use(
  session({
    secret: process.env.SESSION_SECRET, // Change this value to a unique value for your application!
...
fetch("https://us.oauth.battle.net/oauth/token", {
  body: "grant_type=client_credentials",
  method: "POST",
  headers: {
    Authorization: "Basic " + btoa(BNET_ID + ":" + BNET_SECRET),
...
app.delete("/deletePost/:id", (req, res) => {
  ...
  PostModel.findByIdAndDelete(idAPP_LISTEN_PORT)
    .then(function (err) {
      res.json({
              ...
              if (result[i].postId == theResultRefined._id.toHexString()) {
                ...
                if (diff >= 43200000) {
                  ...
                  CommentsModel.deleteMany({
                    postId: theResultRefined._id,
                  }).then(function (result) {
                    console.log(result);
                  });
                  PostModel.findByIdAndDelete(theResultRefined._id).then(
                    function (result) {
                ...
                console.log(
                  result[i].postId,
                  theResultRefined._id.toHexString()
                );
